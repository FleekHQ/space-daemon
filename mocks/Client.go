...
// GetPathAccessRoles provides a mock function with given fields: ctx, b, bucketKey, _a3
func (_m *Client) GetPathAccessRoles(ctx context.Context, b textile.Bucket, bucketKey string, _a3 string) ([]string, error) {
	ret := _m.Called(ctx, b, bucketKey, _a3)

	var r0 []string
	if rf, ok := ret.Get(0).(func(context.Context, textile.Bucket, string, string) []string); ok {
		r0 = rf(ctx, b, bucketKey, _a3)
	} else {
		if ret.Get(0) != nil {
			r0 = ret.Get(0).([]string)
		}
	...
	var r1 error
	if rf, ok := ret.Get(1).(func(context.Context, textile.Bucket, string, string) error); ok {
		r1 = rf(ctx, b, bucketKey, _a3)
	} else {
		r1 = ret.Error(1)
	}
