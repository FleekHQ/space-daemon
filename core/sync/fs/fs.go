...
func (h *Handler) OnCreate(path string, fileInfo os.FileInfo) {
	log.Info("Watcher Handler: OnCreate", fmt.Sprintf("path:%s", path), fmt.Sprintf("fileInfo:%v", fileInfo))
}

func (h *Handler) OnRemove(path string, fileInfo os.FileInfo) {
	log.Info("Watcher Handler: OnRemove", fmt.Sprintf("path:%s", path), fmt.Sprintf("fileInfo:%v", fileInfo))
}

func (h *Handler) OnWrite(path string, fileInfo os.FileInfo) {
	log.Info("Watcher Handler: OnWrite", fmt.Sprintf("path:%s", path), fmt.Sprintf("fileInfo:%v", fileInfo))
}
