...
func (tc *textileClient) SetMember(ctx context.Context, slug string, pk string, msg *string) error {
	iid := uuid.New()

	inviter, err := getCurrentPubKey(tc.store)
	if err != nil {
		return err
	}
...
func getCurrentPubKey(str store.Store) ([]byte, error) {
...
func (tc *textileClient) SetOwner(ctx context.Context, slug string) error {
	pk, err := getCurrentPubKey(tc.store)
	if err != nil {
		return err
	}
